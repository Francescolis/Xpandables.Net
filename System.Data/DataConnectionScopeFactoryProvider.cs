...
public sealed class DataConnectionScopeFactoryProvider(IDataConnectionFactoryProvider factoryProvider) : IDataConnectionScopeFactoryProvider
{
	private readonly IDataConnectionFactoryProvider _factoryProvider = factoryProvider ?? throw new ArgumentNullException(nameof(factoryProvider));
	...
	/// <inheritdoc />
	public Task<IDataConnectionScope> CreateScopeAsync(string name, string providerInvariantName, CancellationToken cancellationToken = default) =>
		GetScopeFactory(name, providerInvariantName).CreateScopeAsync(cancellationToken);

	/// <inheritdoc />
	public IDataConnectionScope CreateScope(string name) =>
		GetScopeFactory(name).CreateScope();

	/// <inheritdoc />
	public IDataConnectionScope CreateScope(string name, string providerInvariantName) =>
