...
/// <param name="dataSqlService">The data SQL service accessor providing the SQL builder and mapper.</param>
...
public class DataRepository<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] TData>(
	IDataConnectionScope connectionScope,
	IDataSqlServiceAccessor dataSqlService,
	IDataCommandInterceptor? interceptor = null)
	: IDataRepository<TData>, IDisposableCheck
	where TData : class
{
	private static readonly string _entityTypeName = typeof(TData).Name;

	/// <summary>
	/// Gets the current data connection scope used for database operations.
	/// </summary>
	/// <remarks>The connection scope manages the lifetime and context of database connections for the
	/// implementing class. This property is intended for use by derived classes to coordinate transactional or scoped
	/// data access.</remarks>
	[SuppressMessage("Usage", "CA2213:Disposable fields should be disposed", Justification = "")]
	protected IDataConnectionScope ConnectionScope { get; } = connectionScope ?? throw new ArgumentNullException(nameof(connectionScope));
	private readonly IDataSqlBuilder _sqlBuilder = dataSqlService.DataSqlBuilder ?? throw new ArgumentNullException(nameof(dataSqlService));
	private readonly IDataSqlMapper _sqlMapper = dataSqlService.DataSqlMapper ?? throw new ArgumentNullException(nameof(dataSqlService));
	private readonly IDataCommandInterceptor _interceptor = interceptor ?? DataCommandInterceptor.Default;
	private bool _isDisposed;

#pragma warning disable CA1033 // Interface methods should be callable by child types
	bool IDisposableCheck.IsDisposed => _isDisposed;
#pragma warning restore CA1033 // Interface methods should be callable by child types
		...
		[EnumeratorCancellation] CancellationToken cancellationToken = default)
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildSelect(specification);
		...
		DbCommand command = ConnectionScope.CreateCommand(queryResult);
		await using (command.ConfigureAwait(false))
		{
			DbDataReader reader = await ExecuteReaderInterceptedAsync(command, context, cancellationToken)
		...
		CancellationToken cancellationToken = default)
	{
		...
		async IAsyncEnumerable<TResult> ReadPagedAsync([EnumeratorCancellation] CancellationToken ct = default)
		{
			...
			DbCommand command = ConnectionScope.CreateCommand(selectResult);
			await using (command.ConfigureAwait(false))
			{
		...
		CancellationToken cancellationToken = default)
	{
		...
		await foreach (TResult? result in QueryAsync(specification, cancellationToken).ConfigureAwait(false))
		{
			count++;
			if (count == 1)
			{
		...
		CancellationToken cancellationToken = default)
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildCount(specification);
		...
		await using (command.ConfigureAwait(false))
		{
			object? result = await ExecuteScalarInterceptedAsync(command, context, cancellationToken).ConfigureAwait(false);
	...
	public virtual async Task<bool> ExistsAsync<TResult>(
		...
		CancellationToken cancellationToken = default)
	{
		long count = await CountAsync(specification, cancellationToken).ConfigureAwait(false);
		return count > 0;
	}
		...
		[EnumeratorCancellation] CancellationToken cancellationToken = default)
	{
		...
		DbCommand command = ConnectionScope.CreateCommand();
		...
		[EnumeratorCancellation] CancellationToken cancellationToken = default)
	{
		...
		DataSpecification<TData, TResult> specification = DataSpecification.For<TData>().Select(selector);
		...
		CancellationToken cancellationToken = default)
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildInsert(data);
			...
			CancellationToken cancellationToken = default)
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildInsertBatch(dataList);
	...
	/// <inheritdoc/>
	public virtual async Task<TIdentity> InsertAsync<TIdentity>(
		TData entity,
		CancellationToken cancellationToken = default)
		where TIdentity : struct
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildInsertWithIdentity<TData, TIdentity>(entity);
		var context = new DataCommandContext(queryResult.Sql, queryResult.Parameters, DataCommandOperationType.Scalar, _entityTypeName);
		...
		CancellationToken cancellationToken = default)
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildUpdate(specification, updater);
		var context = new DataCommandContext(queryResult.Sql, queryResult.Parameters, DataCommandOperationType.NonQuery, _entityTypeName);
		...
		CancellationToken cancellationToken = default)
	{
		...
		SqlQueryResult queryResult = _sqlBuilder.BuildDelete(specification);
		var context = new DataCommandContext(queryResult.Sql, queryResult.Parameters, DataCommandOperationType.NonQuery, _entityTypeName);
	...
	{
		await ConnectionScope.EnsureOpenAsync(cancellationToken).ConfigureAwait(false);
