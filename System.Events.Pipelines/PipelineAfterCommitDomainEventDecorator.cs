...
public sealed class PipelineAfterCommitDomainEventDecorator<TRequest>() :
	...
	where TRequest : class, IRequest, IRequiresEventStorage
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		{
			foreach (PendingDomainEventsBatch batch in PipelineBeforeCommitDomainEventDecorator<TRequest>.DomainEventCommitBuffer.Drain())
			{
				batch.OnCommitted();
			}
		}
