...
/// </summary>
/// <remarks>This registry is immutable after construction. Wrappers are resolved from the service provider
/// at construction time, avoiding captive dependency issues. Attempting to register multiple wrappers for the same
/// event type will result in only one being stored for that type.</remarks>
/// <param name="serviceProvider">The service provider used to resolve registered <see cref="IEventHandlerWrapper"/> instances.</param>
public sealed class EventHandlerRegistry(IServiceProvider serviceProvider) : IEventHandlerRegistry
{
	private readonly ImmutableDictionary<Type, IEventHandlerWrapper> _mapper =
		serviceProvider.GetServices<IEventHandlerWrapper>()
			.ToImmutableDictionary(w => w.EventType);
...
/// </summary>
/// <remarks>This registry is immutable after construction and is safe for concurrent access. Wrappers are
/// resolved from the service provider at construction time. Attempting to register multiple wrappers for
/// the same event type will result in only one being stored.</remarks>
/// <param name="serviceProvider">The service provider used to resolve registered <see cref="IEventHandlerWrapper"/> instances.</param>
public sealed class StaticEventHandlerRegistry(IServiceProvider serviceProvider) : IEventHandlerRegistry
{
	private readonly ImmutableDictionary<Type, IEventHandlerWrapper> _mapper =
		serviceProvider.GetServices<IEventHandlerWrapper>()
			.ToImmutableDictionary(w => w.EventType);
...
public sealed class DynamicEventHandlerRegistry : IEventHandlerRegistry
{
	...
	{
		var wrapper = new DirectEventHandlerWrapper<TEvent>(handlers);
