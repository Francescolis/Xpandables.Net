    ...
    where TRequest : class, IRequest
{
    ...
    Task<OperationResult> IRequestHandler<TRequest>.HandleAsync(
    ...
    where TRequest : class, IRequest<TResponse>
{
    ...
    /// cref="OperationResult{TResponse}"/> representing the outcome of the request execution.</returns>
    Task<OperationResult<TResponse>> HandleAsync(
        RequestContext<TRequest> context,
        CancellationToken cancellationToken = default);

    [EditorBrowsable(EditorBrowsableState.Never)]
    async Task<OperationResult<TResponse>> IRequestHandler<TRequest, TResponse>.HandleAsync(
        TRequest request, CancellationToken cancellationToken) =>
    ...
    where TRequest : class, IStreamRequest<TResponse>
{
    ...
    /// <param name="cancellationToken">A token to monitor for cancellation requests. The default value is <see cref="CancellationToken.None"/>.</param>
    /// <returns>A task that represents the asynchronous operation. The task result contains an <see cref="OperationResult{T}"/>
    ...
    Task<OperationResult<IAsyncPagedEnumerable<TResponse>>> HandleAsync(
        RequestContext<TRequest> context,
        CancellationToken cancellationToken = default);

    [EditorBrowsable(EditorBrowsableState.Never)]
    async Task<OperationResult<IAsyncPagedEnumerable<TResponse>>> IStreamRequestHandler<TRequest, TResponse>.HandleAsync(
        TRequest request, CancellationToken cancellationToken) =>
        await HandleAsync(new(request), cancellationToken).ConfigureAwait(false);
}