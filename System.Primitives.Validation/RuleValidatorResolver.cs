...
/// </summary>
/// <remarks>Use this class to obtain an <see cref="IRuleValidator{TArgument}"/> instance from a service provider for
...
public sealed class RuleValidatorResolver<TArgument> : IRuleValidatorResolver
    where TArgument : class, IRequiresValidation
{
    ...
    /// <inheritdoc/>
    public IRuleValidator? Resolve(IServiceProvider serviceProvider)
    {
        var validators = serviceProvider.GetServices<IRuleValidator<TArgument>>().ToList();
        Type builtInValidatorType = typeof(RuleValidator<TArgument>);
