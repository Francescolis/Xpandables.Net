...
/// <summary>
/// Provides access to the current correlation context for the logical execution flow using asynchronous local storage.
/// </summary>
/// <remarks>
/// <para>This accessor ensures that the correlation context is preserved across asynchronous calls within the same
/// logical operation. It is suitable for scenarios where correlation context must flow with async operations, such as in web
...
/// <see cref="CorrelationContextScopeExtensions.BeginScope"/> to ensure the context is restored (or cleared)
...
public sealed class AsyncLocalCorrelationContextAccessor : ICorrelationContextAccessor
{
	...
	/// <inheritdoc/>
	/// <remarks>Returns the default <see cref="CorrelationContext"/> when no scope is active.</remarks>
	public CorrelationContext Current => _current.Value ?? default;

	/// <summary>
	/// Sets the current correlation context for the logical execution flow.
	/// </summary>
	/// <param name="context">The correlation context to associate with the current flow.</param>
	[Diagnostics.CodeAnalysis.SuppressMessage("Performance", "CA1822:Mark members as static", Justification = "<Pending>")]
	internal void SetCurrent(CorrelationContext context) => _current.Value = context;

	/// <summary>
	/// Clears the current correlation context, removing any value from the async-local slot.
	/// </summary>
	/// <remarks>
	...
	/// In ASP.NET Core middleware, prefer <see cref="CorrelationContextScopeExtensions.BeginScope"/> instead,
