...
public sealed class CacheTypeResolver : Disposable, ICacheTypeResolver
{
	...
	public static void RegisterAssemblies(params IEnumerable<Assembly> assemblies)
	{
	...
	[RequiresUnreferencedCode("Uses reflection to load types from assemblies.")]
	public static void RegisterAssemblies(Predicate<Type> predicate, params IEnumerable<Assembly> assemblies)
	{
		ArgumentNullException.ThrowIfNull(predicate);

		Assembly[] assembliesArray = assemblies as Assembly[] ?? [.. assemblies];

		_assemblies = assembliesArray is { Length: > 0 }
			? assembliesArray
