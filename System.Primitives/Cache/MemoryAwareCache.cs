	...
	where TValue : class
{
	...
	public TValue GetOrAdd(TKey key, Func<TKey, TValue> factory)
	{
		...
		if (_cache.TryGetValue(key, out WeakCacheEntry<TValue> entry)
			&& entry.TryGetValue(out TValue? existingValue))
		{
	...
	public bool TryGetValue(TKey key, out TValue? value)
	{
		value = null;
		return !_disposed
			&& _cache.TryGetValue(key, out WeakCacheEntry<TValue> entry)
	...
	private void Cleanup(object? state)
	{
		...
		foreach (KeyValuePair<TKey, WeakCacheEntry<TValue>> kvp in _cache)
		{
			WeakCacheEntry<TValue> entry = kvp.Value;
