...
public static class ObjectExtensions
{
	extension(object? obj)
	{
		...
		public object? ChangeTypeNullable([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] Type conversionType, IFormatProvider? formatProvider = null)
		{
			...
			if (TryConvertKeyValuePair(obj, conversionType, formatProvider, out object? kvpResult))
			{
				return kvpResult;
			}

			if (TryConvertImmutableArray(obj, conversionType, formatProvider, out object? immutableArrayResult))
			{
				return immutableArrayResult;
			}

			if (TryConvertImmutableCollection(obj, conversionType, formatProvider, out object? immutableResult))
			{
				return immutableResult;
			}

			if (TryConvertEnumerable(obj, conversionType, formatProvider, out object? enumerableResult))
			{
				return enumerableResult;
			}

			if (TryConvertDictionary(obj, conversionType, formatProvider, out object? dictResult))
			{
				return dictResult;
			}

			if (TryConvertExpando(obj, conversionType, out object? expandoResult))
			{
				return expandoResult;
			}

			if (TryConvertSpecialTypes(obj, conversionType, formatProvider, out object? specialResult))
			{
	...
	private static bool TryConvertEnumerable(object obj, Type conversionType, IFormatProvider? formatProvider, out object? result)
	{
		...
		foreach (string part in parts)
		{
			...
			object? converted = ChangeTypeNullable(trimmed, elementType, formatProvider);
			list.Add(converted);
		}
	...
	private static bool TryConvertImmutableCollection(object obj, Type conversionType, IFormatProvider? formatProvider, out object? result)
	{
		...
		Type gen = conversionType.GetGenericTypeDefinition();
		Type[] args = conversionType.GetGenericArguments();

		// ImmutableList<T>
		if (gen == typeof(ImmutableList<>))
		{
			if (!TryConvertEnumerable(obj, typeof(IEnumerable<>).MakeGenericType(args[0]), formatProvider, out object? enumerable))
			{
				return false;
			}

			MethodInfo method = typeof(ImmutableList)
		...
		if (gen == typeof(ImmutableHashSet<>))
		{
			...
			MethodInfo method = typeof(ImmutableHashSet)
				.GetMethod("CreateRange", [typeof(IEnumerable<>).MakeGenericType(args[0])])!
				.MakeGenericMethod(args[0]);
		...
		{
			if (!TryConvertDictionary(obj, conversionType, formatProvider, out object? dictObj))
			{
				return false;
			}
			...
			MethodInfo createRange = typeof(ImmutableDictionary)
	...
	private static bool TryConvertImmutableArray(object obj, Type conversionType, IFormatProvider? formatProvider, out object? result)
	{
		...
		MethodInfo builderType = typeof(ImmutableArray).GetMethod("CreateBuilder", [typeof(int)])!
			.MakeGenericMethod(elementType);

		object builder = builderType.Invoke(null, [0])!;

		MethodInfo addMethod = builder.GetType().GetMethod("Add")!;
		...
		MethodInfo toImmutableMethod = builder.GetType().GetMethod("ToImmutable")!;
	...
	private static bool TryConvertExpando(object obj, Type targetType, out object? result)
	{
		...
		if (obj is string json && LooksLikeJson(json))
		{
			try
			{
				Dictionary<string, object?>? dict = Text.Json.JsonSerializer.Deserialize<Dictionary<string, object?>>(json);
				if (dict is null)
				{
					return false;
				}
	...
	private static bool TryConvertKeyValuePair(object obj, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] Type targetType, IFormatProvider? formatProvider, out object? result)
	{
		...
		object? key = ChangeTypeNullable(parts[0].Trim(), keyType, formatProvider);
		object? value = ChangeTypeNullable(parts[1].Trim(), valueType, formatProvider);
	...
	private static bool TryConvertDictionary(object obj, Type targetType, IFormatProvider? formatProvider, out object? result)
	{
		...
		foreach (string pair in pairs)
		{
			...
			object? key = ChangeTypeNullable(kv[0].Trim(), keyType, formatProvider);
			object? value = ChangeTypeNullable(kv[1].Trim(), valueType, formatProvider);
