...
/// </summary>
/// <remarks>This interface extends <see cref="IRequestStreamHandler{TRequest, TResponse}"/> by providing
/// additional context-aware handling capabilities through the <see cref="RequestContext{TRequest}"/>.</remarks>
/// <typeparam name="TRequest">The type of the request being handled. Must implement <see cref="IRequestStream{TResponse}"/>.</typeparam>
/// <typeparam name="TResponse">The type of the response elements produced by the stream.</typeparam>
public interface IRequestContextStreamHandler<TRequest, TResponse> : IRequestStreamHandler<TRequest, TResponse>
	where TRequest : class, IRequestStream<TResponse>
{
	...
	[EditorBrowsable(EditorBrowsableState.Never)]
	async Task<Result<IAsyncEnumerable<TResponse>>> IRequestStreamHandler<TRequest, TResponse>.HandleAsync(
		TRequest request, CancellationToken cancellationToken) =>
		await HandleAsync(new(request), cancellationToken).ConfigureAwait(false);
}
