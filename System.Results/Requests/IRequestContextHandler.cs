    ...
    where TRequest : class, IStreamRequest<TResponse>
{
    ...
    Task<Result<IAsyncEnumerable<TResponse>>> HandleAsync(
        RequestContext<TRequest> context,
        CancellationToken cancellationToken = default);

    [EditorBrowsable(EditorBrowsableState.Never)]
    async Task<Result<IAsyncEnumerable<TResponse>>> IStreamRequestHandler<TRequest, TResponse>.HandleAsync(
        TRequest request, CancellationToken cancellationToken) =>
        await HandleAsync(new(request), cancellationToken).ConfigureAwait(false);
}
...
/// </summary>
/// <remarks>This interface extends <see cref="IStreamPagedRequestHandler{TRequest, TResponse}"/> by providing
/// additional context-aware handling capabilities through the <see cref="RequestContext{TRequest}"/>.</remarks>
/// <typeparam name="TRequest">The type of the request being handled. Must implement <see cref="IStreamPagedRequest{TResponse}"/>.</typeparam>
/// <typeparam name="TResponse">The type of the response elements produced by the stream.</typeparam>
public interface IStreamPagedRequestContextHandler<TRequest, TResponse> : IStreamPagedRequestHandler<TRequest, TResponse>
    where TRequest : class, IStreamPagedRequest<TResponse>
{
    ...
    [EditorBrowsable(EditorBrowsableState.Never)]
    async Task<Result<IAsyncPagedEnumerable<TResponse>>> IStreamPagedRequestHandler<TRequest, TResponse>.HandleAsync(
        TRequest request, CancellationToken cancellationToken) =>
        await HandleAsync(new(request), cancellationToken).ConfigureAwait(false);
}