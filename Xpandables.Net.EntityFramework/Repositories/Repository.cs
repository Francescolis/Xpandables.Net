    ...
    where TEntity : class, IEntity
{
    ...
    public virtual async ValueTask<TEntity?> TryFindByKeyAsync<TKey>(
        ...
        => await Context.Set<TEntity>().FindAsync([key], cancellationToken: cancellationToken)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var queryableResult = typeof(IEntity).IsAssignableFrom(typeof(TResult))
            ? filter.GetQueryableFiltered(Context.Set<TEntity>())
            : filter.GetQueryableFiltered(Context.Set<TEntity>().AsNoTracking());

        return await queryableResult.FirstOrDefaultAsync(cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await filter.GetQueryableFiltered(Context.Set<TEntity>().AsNoTracking())
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var deleteTableResult = filter.GetQueryableFiltered(Context.Set<TEntity>()).AsEnumerable();
