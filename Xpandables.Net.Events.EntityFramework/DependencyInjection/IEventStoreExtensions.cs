...
namespace Xpandables.Net.Events.DependencyInjection;
...
public static class IEventStoreExtensions
{
    ...
    extension(IServiceCollection services)
    {
        ...
        public IServiceCollection AddXEventUnitOfWork()
        {
            ArgumentNullException.ThrowIfNull(services);
        ...
        /// <typeparam name="TEventUnitOfWork">The event unit of work type to register. Must be a class implementing <see cref="IEventUnitOfWork"/> and have a public
        /// constructor.</typeparam>
        /// <returns>The same <see cref="IServiceCollection"/> instance, to allow for method chaining.</returns>
        public IServiceCollection AddXEventUnitOfWork<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TEventUnitOfWork>()
            where TEventUnitOfWork : class, IEventUnitOfWork
        {
            ArgumentNullException.ThrowIfNull(services);
            return services.AddScoped<IEventUnitOfWork, TEventUnitOfWork>();
        }
