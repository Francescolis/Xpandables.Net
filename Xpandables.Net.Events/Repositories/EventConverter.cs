...
public abstract class EventConverter : IEventConverter
{
    ...
    /// details.</exception>
    [RequiresUnreferencedCode("May use unreferenced code to serialize type.")]
    [RequiresDynamicCode("May use dynamic code to serialize type.")]
         ...
         JsonDocumentOptions documentOptions = default)
    {
        ...
        try
        {
            return JsonSerializer.SerializeToDocument(eventInstance, serializerOptions);
        }
    ...
    [RequiresUnreferencedCode("May use unreferenced code to deserialize type.")]
    [RequiresDynamicCode("May use dynamic code to deserialize type.")]
    ...
    [RequiresUnreferencedCode("May use unreferenced code to deserialize JSON.")]
    [RequiresDynamicCode("May use dynamic code to deserialize JSON.")]
    ...
    public static Func<JsonDocument, IEvent> ResolveEventDeserializer(Type type, JsonSerializerOptions options)
    {
        ...
        if (options.TypeInfoResolver?.GetTypeInfo(type, options) is not JsonTypeInfo info)
        {
            return json =>
            {
                object? @event = json.Deserialize(type, options)
                    ?? throw new InvalidOperationException(
                        $"Failed to deserialize the event data to {type.Name}.");

                return (IEvent)@event;
            };
        }

        var key = (info, options);
        ...
        return _deserializerCache.GetOrAdd(key, static k =>
        {
            ...
            ?? throw new InvalidOperationException(
                $"No deserializer found for the event {info.Type.Name}");
    }
