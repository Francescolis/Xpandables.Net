...
public sealed class RestResponseBuilder(IServiceProvider serviceProvider) : IRestResponseBuilder
{
        ...
        RestResponseContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (context.Request.ResultType is not null)
        {
            var genericType = typeof(IRestResponseResultComposer<>).MakeGenericType(context.Request.ResultType);
            var genericComposers = serviceProvider.GetServices(genericType).Cast<IRestResponseComposer>();
            composers = genericComposers.Concat(composers);
        }
