...
/// and applies all validators found for the target query before the command get handled.
...
/// <remarks>
/// Initializes a new instance of the <see cref="ValidatorAsyncQueryDecorator{TQuery, TResult}"/> class with
...
public sealed class ValidatorAsyncQueryDecorator<TQuery, TResult>(
    ...
    ICompositeValidator<TQuery> validator) : IAsyncQueryHandler<TQuery, TResult>
    where TQuery : notnull, IAsyncQuery<TResult>, IValidateDecorator
{
        ...
        TQuery query, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (TResult result in _decoratee
