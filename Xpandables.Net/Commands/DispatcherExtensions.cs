...
public static class DispatcherExtensions
{
    ...
    /// <exception cref="ArgumentNullException">The <paramref name="query"/> 
    /// is null.</exception>
    ...
    /// using the <see cref="IAsyncQueryHandler{TQuery, TResult}"/> 
    /// implementation and returns an enumerator of 
    /// <typeparamref name="TResult"/> that can be asynchronously enumerated.
    /// </summary>
    /// <typeparam name="TResult">Type of the result.</typeparam>
    /// <param name="dispatcher">The target dispatcher instance.</param>
    /// <param name="query">The query to act on.</param>
    ...
    /// failed.</exception>
    /// <returns>An enumerator of <typeparamref name="TResult"/> that can be 
        ...
        IAsyncQuery<TResult> query, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            IAsyncQueryHandlerWrapper<TResult> handler =
                (IAsyncQueryHandlerWrapper<TResult>)dispatcher
                .GetRequiredService(queryWrapperType);
