...
/// <summary>
/// Builds cookies from the request context if the location is set to Cookie. It adds each cookie to the message
/// options.
/// </summary>
public sealed class RestRequestCookieBuilder : RestRequestBuilder<IRestContentCookie>
{
    ///<inheritdoc/>
    public override void Build(RestRequestContext context)
    {
        ...
        IRestContentCookie request = (IRestContentCookie)context.Request;

        IDictionary<string, object?> cookieSource
             = request.GetCookieHeaderValue();
