...
/// before request execution. You must register the <see cref="AuthenticationHeaderValueProvider"/> provider.
/// </summary>
/// <remarks>
...
public sealed class HttpClientAuthorizationHandler(AuthenticationHeaderValueProvider providerDelegate)
    : HttpClientHandler
{
    private readonly AuthenticationHeaderValueProvider _providerDelegate = providerDelegate
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!request.Options.TryGetValue(new(nameof(HttpClientAttribute.IsSecured)), out bool? isSecured) || !isSecured.GetValueOrDefault())
        ...
        if (headerValue is null)
            throw new InvalidOperationException(
                $"Expected authorization value not provided or applied by the {nameof(AuthenticationHeaderValueProvider)} instance.");
