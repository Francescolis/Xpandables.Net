...
internal sealed class HttpClientRequestBuilder(IServiceProvider serviceProvider) : IHttpClientRequestBuilder
{
        ...
        where TSource : class
    {
        ...
        static HttpClientAttribute ReadHttpRestClientAttribute(TSource source, IServiceProvider serviceProvider)
        {
            if (source is IHttpClientAttributeProvider httpRestClientAttributeProvider)
                return httpRestClientAttributeProvider.GetHttpClientAttribute(serviceProvider);
            ...
            return source.GetType().GetCustomAttribute<HttpClientAttribute>()
                ?? throw new ArgumentNullException(
                    ...
                    $"attribute or implement {nameof(IHttpClientAttributeProvider)} interface.");
        }
    }
        ...
        where TSource : class
    {
        ...
        {
            string headerValue = headerSource.Select(x => $"{x.Key},{x.Value.StringJoin(";")}").StringJoin(",");
