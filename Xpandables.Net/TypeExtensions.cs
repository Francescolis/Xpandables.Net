...
using static Xpandables.Net.Expressions.ExpressionExtensions;
...
public static class TypeExtensions
{
    ...
    public static string GetNameWithoutGenericArity(this Type type)
    {
        ...
        int index = type.GetTypeInfo().Name
            .IndexOf('`', StringComparison.OrdinalIgnoreCase);
        return index == -1 ? type
            .GetTypeInfo().Name : type.GetTypeInfo().Name[..index];
    }
    ...
    /// <returns><see langword="true"/> if so, 
    ...
    /// <returns>The description string. If not found,
    ...
    public static bool IsOverridden(this MethodInfo methodInfo)
    {
        ...
        return methodInfo
            .GetBaseDefinition().DeclaringType != methodInfo.DeclaringType;
    }
    ...
    public static bool IsAwaitable(this MethodInfo methodInfo)
    {
        ...
        return methodInfo.ReturnType.
            GetMethod(nameof(Task.GetAwaiter)) is not null;
    }
    ...
    /// <summary>
    /// Determines whether the current type implements 
    /// or it's <see cref="IAsyncEnumerable{T}"/>.
    /// </summary>
    /// <param name="type">The type to act on.</param>
    /// <returns><see langword="true"/> if found, otherwise 
    /// <see langword="false"/>.</returns>
    ...
    /// <param name="assemblyException">The handled exception d
    /// uring assembly loading if fails.</param>
    /// <returns>Returns <see langword="true"/> if loading OK 
    ...
    /// <param name="loadedAssembly">The loaded assembly if succeeded.</param>
    /// <param name="assemblyException">The handled exception 
    /// during assembly loading.</param>
    /// <returns>Returns <see langword="true"/> if loading OK and 
    ...
    /// <summary>
    /// Tries to invoke the specified member, using 
    /// the specified binding constraints and matching
    /// the specified argument list.
    /// </summary>
    /// <param name="type"></param>
    /// <param name="result">An object representing the 
    /// return value of the invoked member
    /// or an empty result with handled exception.</param>
    /// <param name="invokeException">The handled invoke exception.</param>
    /// <param name="memberName">The string containing the name of 
    /// the constructor, method, property, or field
    ...
    /// <param name="invokeAttr">A bit-mask comprised of one or 
    /// more System.Reflection.BindingFlags that specify
    /// how the search is conducted. The access can be one of the
    /// BindingFlags such as
    /// Public, NonPublic, Private, InvokeMethod, GetField, and so on. 
    /// The type of lookup
    /// need not be specified. If the type of lookup is omitted, BindingFlags.
    /// Public
    /// | BindingFlags.Instance | BindingFlags.Static are used.</param>
    /// <param name="binder">An object that defines a set of properties 
    /// and enables binding, which can involve
    /// selection of an overloaded method, coercion of argument types, 
    /// and invocation
    /// of a member through reflection. /// -or- /// A null reference 
    /// (Nothing in Visual
    /// Basic), to use the System.Type.DefaultBinder. Note that explicitly defining a
    /// System.Reflection.Binder object may be required for successfully 
    /// invoking method
    /// overloads with variable arguments.</param>
    /// <param name="target">The object on which to invoke the specified member, 
    /// if available.</param>
    /// <param name="args">An array containing the arguments to pass to 
    /// the member to invoke.</param>
    /// <returns>Returns <see langword="true"/> 
    /// if invoke OK and <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="type"/> is null.</exception>
    /// <exception cref="ArgumentNullException">The 
    ...
    /// <summary>
    /// Tries to substitute the elements of an array of types for 
    /// the type parameters of the
    /// current generic type definition and returns a System.Type 
    /// object representing
    ...
    /// <param name="typeArguments">An array of types to be substituted 
    /// for the type parameters of the current generic
    /// type.</param>
    /// <returns>Returns <see langword="true"/> 
    /// if make OK and <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="type"/> is null.</exception>
    ...
    /// <returns>Returns <see langword="true"/> if make OK and 
    /// <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="type"/> is null.</exception>
    /// <exception cref="ArgumentNullException">The 
        ...
        where TDelegate : Delegate
    {
        ...
        if (parameterTypes.Length == 0)
        {
            try
            {
                ...
                constructorDelegate = Expression.Lambda<TDelegate>(body)
                    .Compile();
        ...
        if (!type.TryGetConstructorInfo(
            out ConstructorInfo? constructor,
            out constructorException,
            parameterTypes))
        {
            constructorDelegate = default;
            return false;
        ...
        ParameterExpression[] parameterExpressions
            = GetParameterExpression(parameterTypes);

        if (!constructor.TryGetConstructorExpression(
            out Expression? constructorExpression,
            out constructorException,
        ...
        try
        {
            ...
            when (exception is ArgumentException or ArgumentNullException)
        {
            constructorDelegate = default!;
    ...
    /// <summary>
    /// Tries to get an Expression representing the constructor call, 
    /// passing in the constructor parameters.
    /// </summary>
    /// <param name="constructorInfo">The constructor info to act on.</param>
    /// <param name="constructorExpression">The built constructor 
    /// expression.</param>
    /// <param name="constructorException">The handled constructor 
    /// exception.</param>
    /// <param name="parameterExpressions">A collection of parameter 
    /// expressions.</param>
    /// <returns>Returns <see langword="true"/> if OK 
    /// and <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="constructorInfo"/> is null.</exception>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="parameterExpressions"/> is null.</exception>
        ...
        params Expression[] parameterExpressions)
    {
        ...
        _ = parameterExpressions
            ?? throw new ArgumentNullException(nameof(parameterExpressions));

        try
        {
            constructorException = default;
            constructorExpression = Expression.New(
                constructorInfo,
                parameterExpressions);
            return true;
        }
    ...
    /// <summary>
    /// Tries to get the constructor from the type 
    /// that matches the specified arguments type.
    /// </summary>
    /// <param name="type">The type to act on.</param>
    ...
    /// <returns>Returns <see langword="true"/> if make 
    /// OK and <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="type"/> is null.</exception>
        ...
        params Type[] parameterTypes)
    {
        ...
        try
        {
            ...
            {
                constructorException = new ArgumentNullException(
    ...
    /// <returns>Returns <see langword="true"/> if OK and 
    /// <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="type"/> is null.</exception>
    /// <exception cref="ArgumentNullException">The 
    ...
    /// <summary>
    /// Re-throws the current exception using 
    /// <see cref="ExceptionDispatchInfo.Capture"/> method while preserving stack trace.
    /// </summary>
    ...
    /// <summary>
    /// Checks whether the specified target name exists 
    /// in the <see cref="SerializationInfo"/> instance.
    /// </summary>
    /// <param name="this">The current <see cref="SerializationInfo"/> 
    /// instance to act on.</param>
    /// <param name="target">The string target name to search for.</param>
    /// <returns><see langword="true"/> if found, 
    /// otherwise <see langword="false"/>.</returns>
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="this"/> or <paramref name="target"/> is null.</exception>
    ...
    /// This is similar to the VB.Net key word <see lanwgord="With"/>..
    /// <see lanwgord="EndWith"/>.
    /// </summary>
    ...
    /// This is similar to the VB.Net key word 
    /// <see lanwgord="With"/>..<see lanwgord="EndWith"/>.
    /// </summary>
    /// <typeparam name="TSource">Type source.</typeparam>
    /// <param name="source">The source item to act on.</param>
    ...
    /// <summary>
    /// Sets properties via lambda expression. 
    /// This is useful when dealing with <see cref="Expression{TDelegate}"/>.
    /// </summary>
    /// <typeparam name="TSource">Type source.</typeparam>
    ...
    /// <exception cref="ArgumentException">The 
    /// <paramref name="nameOfExpression"/> is not valid.</exception>
    /// <exception cref="InvalidOperationException">An error occurred 
    /// while setting the property value. See inner exception.</exception>
    public static TSource Assign<TSource>(
        this TSource source,
        ...
        where TSource : class
    {
                                          ...
                                          or TargetInvocationException)
        {
            throw new InvalidOperationException(
                $"Unable to set '{typeof(TSource).Name}.{propertyName}' " +
                $"with the specified value : {value}.", exception);
        }
    ...
    /// <summary>
    /// Assigns the property named <paramref name="propertyName"/> 
    /// with the value <paramref name="value"/> from the specified object <paramref name="source"/>.
    /// </summary>
    /// <typeparam name="TSource">Type source.</typeparam>
    /// <param name="source">The source instance to act on.</param>
    ...
    /// <exception cref="ArgumentNullException">The 
    /// <paramref name="action"/> is null.</exception>
    public static async Task<TSource> AssignAsync<TSource>(
        this TSource source,
    ...
    /// <returns>Returns <see langword="true"/> 
    /// if get OK and <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">The 
        ...
        [MaybeNullWhen(returnValue: true)] out Exception typeException)
    {
        ...
        try
        {
            ...
            if (type is null)
            {
                typeException = new ArgumentNullException(
                    nameof(type), "Type not found.");
                return false;
            }
            return true;
        }
    ...
    /// <param name="typeException">The handled type exception.</param>
    /// <returns>Returns <see langword="true"/> 
    /// if OK and <see langword="false"/> otherwise.</returns>
    /// <exception cref="ArgumentNullException">
    /// The <paramref name="typeName"/> is null.</exception>
        ...
        [MaybeNullWhen(returnValue: true)] out Exception typeException)
    {
        ...
        try
        {
            ...
            when (exception is NotSupportedException or FileNotFoundException)
        {
            typeException = exception;
            return false;
        }
